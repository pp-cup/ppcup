...
async function updateParticipantsPP() {
  ...
  for (let participant of data) {
    try {
      ...
      participant.Points = calculatePoints(parseFloat(participant.PPstart), parseFloat(participant.PPend));
    } catch (err) {
...
app.get('/api/data', (req, res) => {
  const data = loadData();
  data.sort((a, b) => b.Points - a.Points);
  saveData(data); // Сохраняем обновленные позиции в файл
  res.json(data);
});
